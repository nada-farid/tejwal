...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.guide.title_singular') }}
        </div>
                ...
                <div class="row">
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name"
                            id="name" value="{{ old('name', $guide->user->name) }}" required>
                        @if ($errors->has('name'))
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control {{ $errors->has('last_name') ? 'is-invalid' : '' }}" type="text"
                            name="last_name" id="last_name" value="{{ old('last_name', $guide->user->last_name) }}"
                            required>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email"
                            name="email" id="email" value="{{ old('email', $guide->user->email) }}" required>
                        @if ($errors->has('email'))
                            <div class="invalid-feedback">
                                {{ $errors->first('email') }}
                            </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text"
                            name="phone" id="phone" value="{{ old('phone', $guide->user->phone) }}" required>
                        @if ($errors->has('phone'))
                            <div class="invalid-feedback">
                                {{ $errors->first('phone') }}
                            </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control {{ $errors->has('country') ? 'is-invalid' : '' }}" type="text"
                            name="country" id="country" value="{{ old('country', $guide->user->country) }}" required>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control {{ $errors->has('city') ? 'is-invalid' : '' }}" type="text"
                            name="city" id="city" value="{{ old('city', $guide->user->city) }}" required>
                        @if ($errors->has('city'))
                            <div class="invalid-feedback">
                                {{ $errors->first('city') }}
                            </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control date {{ $errors->has('dob') ? 'is-invalid' : '' }}" type="text"
                            name="dob" id="dob" value="{{ old('dob', $guide->user->dob) }}" required>
                        @if ($errors->has('dob'))
                            <div class="invalid-feedback">
                                {{ $errors->first('dob') }}
                            </div>
                    ...
                    <div class="form-group col-md-6">
                        <label class="required"
                            for="naitev_language_id">{{ trans('cruds.user.fields.naitev_language') }}</label>
                        <div style="padding-bottom: 30px">
                        </div>
                        <select class="form-control select2 {{ $errors->has('naitev_language') ? 'is-invalid' : '' }}"
                            name="naitev_language_id" id="naitev_language_id" required>
                            @foreach ($naitev_languages as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('naitev_language_id') ? old('naitev_language_id') : $guide->user->naitev_language->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6">
                        <label class="required"
                            for="speaking_languages">{{ trans('cruds.user.fields.speaking_language') }}</label>
                        ...
                        <select class="form-control select2 {{ $errors->has('speaking_languages') ? 'is-invalid' : '' }}"
                            name="speaking_languages[]" id="speaking_languages" multiple required>
                            @foreach ($speaking_languages as $id => $speaking_language)
                                <option value="{{ $id }}"
                                    {{ in_array($id, old('speaking_languages', [])) || $guide->user->speaking_languages->contains($id) ? 'selected' : '' }}>
                                    {{ $speaking_language }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6">
                        <label class="required"
                            for="brief_intro">{{ trans('cruds.guide.fields.brief_intro') }}</label>
                        <textarea class="form-control {{ $errors->has('brief_intro') ? 'is-invalid' : '' }}"
                            name="brief_intro" id="brief_intro"
                            required>{{ old('brief_intro', $guide->brief_intro) }}</textarea>
                        ...
                        @foreach (App\Models\User::GENDER_RADIO as $key => $label)
                            <div class="form-check {{ $errors->has('gender') ? 'is-invalid' : '' }}">
                                <input class="form-check-input" type="radio" id="gender_{{ $key }}" name="gender"
                                    value="{{ $key }}"
                                    {{ old('gender', $guide->user->gender) === (string) $key ? 'checked' : '' }}
                                    required>
                                <label class="form-check-label"
                                    for="gender_{{ $key }}">{{ trans('global.gender.' . $label) }}</label>
                            </div>
                        @endforeach
                    ...
                    <div class="form-group col-md-6">
                        ...
                        @foreach (App\Models\Guide::DRIVING_LICENCE_RADIO as $key => $label)
                            <div class="form-check {{ $errors->has('driving_licence') ? 'is-invalid' : '' }}">
                                <input class="form-check-input" type="radio" id="driving_licence_{{ $key }}"
                                    name="driving_licence" value="{{ $key }}"
                                    {{ old('driving_licence', $guide->driving_licence) === (string) $key ? 'checked' : '' }}
                                    required>
                                <label class="form-check-label"
                                    for="driving_licence_{{ $key }}">{{ trans('global.driving.' . $label) }}</label>
                            </div>
                        @endforeach
                    ...
                    <div class="form-group col-md-6">
                        ...
                        @foreach (App\Models\Guide::CAR_RADIO as $key => $label)
                            <div class="form-check {{ $errors->has('car') ? 'is-invalid' : '' }}">
                                <input class="form-check-input" type="radio" id="car_{{ $key }}" name="car"
                                    value="{{ $key }}"
                                    {{ old('car', $guide->car) === (string) $key ? 'checked' : '' }} required>
                                <label class="form-check-label"
                                    for="car_{{ $key }}">{{ trans('global.car.' . $label) }}</label>
                            </div>
                        @endforeach
                        @if ($errors->has('car'))
                            <div class="invalid-feedback">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        @foreach (App\Models\Guide::DEGREE_RADIO as $key => $label)
                            <div class="form-check {{ $errors->has('degree') ? 'is-invalid' : '' }}">
                                <input class="form-check-input" type="radio" id="degree_{{ $key }}" name="degree"
                                    value="{{ $key }}"
                                    {{ old('degree', $guide->degree) === (string) $key ? 'checked' : '' }} required>
                                <label class="form-check-label"
                                    for="degree_{{ $key }}">{{ trans('global.degree.' . $label) }}</label>
                            </div>
                        @endforeach
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control {{ $errors->has('major') ? 'is-invalid' : '' }}" type="text"
                            name="major" id="major" value="{{ old('major', $guide->major) }}" required>
                        @if ($errors->has('major'))
                            <div class="invalid-feedback">
                                {{ $errors->first('major') }}
                            </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control {{ $errors->has('cost') ? 'is-invalid' : '' }}" type="number"
                            name="cost" id="cost" value="{{ old('cost', $guide->cost) }}" step="0.01" required>
                        @if ($errors->has('cost'))
                            <div class="invalid-feedback">
                                {{ $errors->first('cost') }}
                            </div>
...
@section('scripts')
    <script>
        Dropzone.options.photoDropzone = {
            ...
            init: function() {
                @if (isset($guide->user) && $guide->user->photo)
                    var file = {!! json_encode($guide->user->photo) !!}
