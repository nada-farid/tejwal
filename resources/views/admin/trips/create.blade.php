            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('trip_categories') ? 'is-invalid' : '' }}" name="trip_categories[]" id="trip_categories" multiple required>
                    @foreach($trip_categories as $id => $trip_category)
                        <option value="{{ $id }}" {{ in_array($id, old('trip_categories', [])) ? 'selected' : '' }}>{{ $trip_category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Trip::CAR_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('car') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="car_{{ $key }}" name="car" value="{{ $key }}" {{ old('car', '0') === (string) $key ? 'checked' : '' }} required>
                        <label class="form-check-label" for="car_{{ $key }}">{{trans('global.'.$label )}}</label>
                    </div>
                @endforeach
                @if($errors->has('car'))
                    <div class="invalid-feedback">
